...
class GenerateEncoderDecoderOutput(ModelOutput):
    ...
    past_key_values: Optional[Tuple[Tuple[Tuple[torch.FloatTensor]]]] = None
    IG_lst: Optional[list] = None
    entropy_student_lst: Optional[list] = None
    entropy_lst: Optional[list] = None
...
class GenerationMixin:
    ...
    def generate(
        ...
        deck_decoding: Optional[bool] = None,
        ...
        **kwargs,
        ...
        if deck_decoding:
            generation_mode = GenerationMode.DECK_GENERATION
        ...
        elif generation_mode == GenerationMode.DECK_GENERATION:
            if self._is_stateful:
                # DoLa decoding was not designed for stateful models, and would require some changes
                raise ValueError(
                    f"dola decoding is not supported with stateful models, such as {self.__class__.__name__}"
                )
            result = self._deck_decoding(
                input_ids,
                input_ids_student,
                alpha=alpha,
                logits_processor=prepared_logits_processor,
    ...
    def _deck_decoding(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            logits = self.relative_top_filter(next_token_logits, relative_top=0.01, min_tokens_to_keep=5) #adjust
            logits_student = next_token_logits_student.log_softmax(dim=-1)
            mask = logits[0] < -1e3
